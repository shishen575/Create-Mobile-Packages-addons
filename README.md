# Create-Mobile-Packages-addons (1.21.1 / NeoForge)

> このブランチは **Minecraft 1.21.1 / NeoForge** 版です。1.20.1 / Forge 版は `main` ブランチを参照してください。
> CMP本体の1.21.1版はAPIが大きく変わっているため、Mixinの実装は1.20.1版と異なります
> （NeoForgeはMojang公式マッピングをそのまま使うため、Forge版で必要だったSRG名の併記は不要）。
...
- **Create Mod** (6.0.7 以上、NeoForge版)
- **Create: Mobile Packages** (0.7.0 以上、NeoForge版) の jar ファイル

### 手順

1. `libs/` フォルダに以下を配置:
   - `create_mobile_packages-1.21.1-0.7.0.jar`
2. ビルド:
   ```bash
   ./gradlew build
   ```
...
`.github/workflows/build.yml` で `mc1.21.1` ブランチへの push / PR / リリース作成時に自動ビルドします。
...
| Secret名 | 値 |
|---|---|
| `CMP_JAR_URL_1211` | `create_mobile_packages-1.21.1-0.7.0.jar` をダウンロードできるURL（自分の private storage 等） |
...
### Mixinに関する重要な注意（NeoForgeはSRG名の心配が不要）

1.20.1/Forge版では配布jar中のヴァニラオーバーライドメソッドがSRG名（`m_5857_`等）に
リネームされている問題があったが、**NeoForgeはMojang公式マッピングをそのまま使う**ため、
このブランチのMixinは人間が読めるメソッド名（`mayPlace`, `quickMoveStack`等）を
そのまま指定するだけでよい（SRG名の併記は不要）。

また、CMP本体の`newRobo`/`newRequestRobo`はどちらも内部で`this.add(robo)`を呼ぶため、
`MixinRoboManager`はその呼び出し1箇所を`@Redirect`で横取りするだけで両方の発進経路に
対応している（ローカル変数キャプチャに依存しないため、配布jarにLVT情報が無くても安全）。
